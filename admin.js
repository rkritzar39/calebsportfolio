...
function setDiscordSyncStatus(message, isError = false) {
    const statusElement = document.getElementById("discord-sync-status");
...
function getDiscordAvatarUrl(discordUser) {
    ...
    if (discordUser.avatar) {
        ...
        return `https://cdn.discordapp.com/avatars/${discordUser.id}/${discordUser.avatar}.${extension}?size=512`;
    }

    const discriminator = Number.parseInt(discordUser.discriminator || "0", 10);
    ...
    if (discriminator > 0) {
        ...
        try {
            defaultAvatarIndex = Number((BigInt(discordUser.id) >> 22n) % 6n);
        } catch {
...
function updateDiscordSyncPreview() {
    const preview = document.getElementById("discord-profile-preview");
...
async function fetchDiscordData(userId = DEFAULT_DISCORD_USER_ID) {
    const normalizedUserId = String(userId || "").trim();

    if (!isValidDiscordUserId(normalizedUserId)) {
        throw new Error("Enter a valid Discord User ID containing 17–20 digits.");
    }

    const response = await fetch(
        `https://api.lanyard.rest/v1/users/${encodeURIComponent(normalizedUserId)}?timestamp=${Date.now()}`,
        {
            cache: "no-store",
    ...
    if (!result?.success || !result?.data?.discord_user) {
        throw new Error("Discord profile data was not available through Lanyard.");
    }
...
/* ============================ */
/* APPLY DISCORD PROFILE        */
/* ============================ */

function applyDiscordDataToProfile() {
    if (!discordData.id) {
        setDiscordSyncStatus("Load the Discord profile before applying it.", true);
        return;
    }
...
function initializeDiscordSyncControls() {
    const syncToggle = document.getElementById("discord-sync-toggle");
    const syncPanel = document.getElementById("discord-sync-panel");
    ...
    syncToggle.dataset.discordSyncInitialized = "true";

    if (userIdInput && !userIdInput.value.trim()) {
        userIdInput.value = DEFAULT_DISCORD_USER_ID;
    }

    const loadAndApplyDiscordProfile = async () => {
        const userId = userIdInput?.value.trim() || DEFAULT_DISCORD_USER_ID;
    ...
    syncToggle.addEventListener("change", async () => {
        ...
        if (!syncToggle.checked) {
            setDiscordSyncStatus("", false);
            return;
        }

        await loadAndApplyDiscordProfile();
    });
...
async function loadProfileData() {
    ...
    try {
        ...
        if (isSync) {
            try {
                await fetchDiscordData(savedDiscordUserId);
            } catch (error) {
                console.warn("Saved Discord profile could not be loaded:", error);
                setDiscordSyncStatus("The saved Discord profile could not be loaded.", true);
            }
        }
        ...
        if (data.countdownTargetDate && typeof data.countdownTargetDate.toDate === "function") {
            countdownDatetimeInput.value = formatLocalCountdownDate(data.countdownTargetDate.toDate());
        } else {
...
async function saveProfileData(event) {
    ...
    if (isSync) {
        try {
            await fetchDiscordData(discordUserId);
        } catch (error) {
            showProfileStatus(`Discord profile could not be loaded: ${error.message}`, true);
            return;
        }
    }
...
document.addEventListener("DOMContentLoaded", () => {
    initializeDiscordSyncControls();

    const autoStatusToggle = document.getElementById("auto-status-toggle");
    const profileStatusDropdown = document.getElementById("profile-status");

    if (autoStatusToggle && profileStatusDropdown &&
        autoStatusToggle.dataset.statusListenerInitialized !== "true") {
        autoStatusToggle.dataset.statusListenerInitialized = "true";

        const updateStatusDropdown = () => {
            profileStatusDropdown.disabled = autoStatusToggle.checked;
            profileStatusDropdown.style.opacity = autoStatusToggle.checked ? "0.6" : "1";
            profileStatusDropdown.style.cursor = autoStatusToggle.checked ? "not-allowed" : "default";
        };
...
/* ======================================================= */
/* ADVANCED EDUCATION PORTFOLIO MANAGER                    */
/* ======================================================= */
const educationPageRef=doc(db,"site_config","educationPage");
const EDU_PRIVACY=["showCumulativeGpa","showSemesterGpa","showCourseGrades","showCompletedCredits","showAttemptedCredits","showPlannedCourses","showTransferCredits","showAchievements","showCertifications","showProjects","showGoals","showAcademicStanding"];
const eduEsc=v=>String(v??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]));
const eduVal=id=>document.getElementById(id)?.value?.trim()||"", eduNum=id=>{const v=eduVal(id);return v===""?null:+v};
function eduStatus(t,bad=false){const e=document.getElementById("education-page-status");if(e){e.textContent=t;e.className=`status-message ${bad?"error":"success"}`}}
function jsonRead(id){const v=eduVal(id);if(!v)return[];const x=JSON.parse(v);if(!Array.isArray(x))throw Error(`${id} must contain a JSON array.`);return x}function jsonSet(id,v){const e=document.getElementById(id);if(e)e.value=JSON.stringify(Array.isArray(v)?v:[],null,2)}
function courseEditor(c={}){const e=document.createElement("div");e.className="advanced-course-row";e.innerHTML=`<input class="ec-code" placeholder="Code" value="${eduEsc(c.code)}"><input class="ec-name" placeholder="Course title" value="${eduEsc(c.name)}"><input class="ec-credits" type="number" min="0" step=".5" placeholder="Credits" value="${eduEsc(c.credits??"")}"><select class="ec-mode"><option value="letter">Standard Letter</option><option value="grade-no-credit">Grade / No Credit</option><option value="pass-no-credit">Pass / No Credit</option><option value="satisfactory-unsatisfactory">Satisfactory / Unsatisfactory</option><option value="audit">Audit</option><option value="transfer">Transfer</option><option value="custom">Custom</option></select><input class="ec-grade" placeholder="Grade: A, PS, S, AU…" value="${eduEsc(c.grade)}"><select class="ec-status"><option>Planned</option><option>In Progress</option><option>Completed</option><option>Transfer</option><option>Withdrawn</option><option>Dropped</option></select><input class="ec-subject" placeholder="Subject / requirement" value="${eduEsc(c.subject||c.requirementArea)}"><input class="ec-skills" placeholder="Skills (comma separated)" value="${eduEsc(Array.isArray(c.skills)?c.skills.join(", "):c.skills)}"><select class="ec-visibility"><option value="public">Public</option><option value="no-grade">Public without grade</option><option value="private">Private</option></select><button type="button" class="danger-btn">Remove</button>`;e.querySelector(".ec-mode").value=c.gradingMode||"letter";e.querySelector(".ec-status").value=c.status||"Planned";e.querySelector(".ec-visibility").value=c.visibility||"public";e.querySelector("button").onclick=()=>e.remove();return e}
function semesterEditor(s={}){const e=document.createElement("article");e.className="advanced-semester-editor";e.innerHTML=`<div class="semester-admin-head"><input class="es-name" placeholder="Fall 2026" value="${eduEsc(s.name)}"><input class="es-dates" placeholder="Aug 31 – Dec 18, 2026" value="${eduEsc(s.dates)}"><select class="es-status"><option>Planned</option><option>Current</option><option>Completed</option><option>Archived</option></select><label><input class="es-current" type="checkbox" ${s.current?"checked":""}> Current</label><button type="button" class="danger-btn remove-sem">Remove semester</button></div><div class="advanced-course-list"></div><button type="button" class="secondary-button add-course">+ Add Course</button>`;e.querySelector(".es-status").value=s.status||"Planned";const list=e.querySelector(".advanced-course-list");(s.courses||[]).forEach(c=>list.appendChild(courseEditor(c)));e.querySelector(".add-course").onclick=()=>list.appendChild(courseEditor());e.querySelector(".remove-sem").onclick=()=>e.remove();return e}
function collectSemesters(){return[...document.querySelectorAll(".advanced-semester-editor")].map(e=>({name:e.querySelector(".es-name").value.trim(),dates:e.querySelector(".es-dates").value.trim(),status:e.querySelector(".es-status").value,current:e.querySelector(".es-current").checked,courses:[...e.querySelectorAll(".advanced-course-row")].map(c=>({code:c.querySelector(".ec-code").value.trim(),name:c.querySelector(".ec-name").value.trim(),credits:+c.querySelector(".ec-credits").value||0,gradingMode:c.querySelector(".ec-mode").value,grade:c.querySelector(".ec-grade").value.trim(),status:c.querySelector(".ec-status").value,subject:c.querySelector(".ec-subject").value.trim(),skills:c.querySelector(".ec-skills").value.split(",").map(x=>x.trim()).filter(Boolean),visibility:c.querySelector(".ec-visibility").value})).filter(c=>c.code||c.name)})).filter(s=>s.name||s.courses.length)}
function renderAdvancedEducation(d={}){const set=(id,v="")=>{const e=document.getElementById(id);if(e)e.value=v??""},p=d.profile||{},deg=d.degree||{};document.getElementById("education-public").checked=d.isPublic!==false;set("education-publish-status",d.publishStatus||"Draft");set("education-institution",p.institution);set("education-college",p.college);set("education-program",p.program);set("education-intended-program",p.intendedProgram);set("education-major",p.major);set("education-minor",p.minor);set("education-concentration",p.concentration);set("education-academic-year",p.academicYear);set("education-enrollment-status",p.enrollmentStatus);set("education-current-term",d.currentTerm);set("education-expected-graduation",p.expectedGraduation);set("education-degree-name",deg.name);set("education-required-credits",deg.requiredCredits??120);set("education-cumulative-gpa",d.cumulativeGpa);set("education-credits-completed",d.creditsCompleted);set("education-intro",d.intro);document.getElementById("education-privacy-grid").innerHTML=EDU_PRIVACY.map(k=>`<label><input type="checkbox" data-edu-privacy="${k}" ${d.privacy?.[k]!==false?"checked":""}> ${k.replace(/([A-Z])/g," $1").replace(/^show /i,"Show ")}</label>`).join("");const sem=document.getElementById("education-semesters-editor");sem.innerHTML="";(d.semesters||[]).forEach(x=>sem.appendChild(semesterEditor(x)));[["requirements",d.requirements],["timeline",d.timeline||d.milestones],["goals",d.goals],["planned",d.plannedCourses],["achievements",d.achievements],["certifications",d.certifications],["projects",d.projects],["skills",d.skills],["transfers",d.transferCredits]].forEach(([id,v])=>jsonSet(`education-${id}-json`,v))}
async function loadAdvancedEducation(){try{const s=await getDoc(educationPageRef);renderAdvancedEducation(s.exists()?s.data():{isPublic:false})}catch(e){console.error(e);eduStatus("Could not load Education portfolio.",true)}}
document.addEventListener("DOMContentLoaded",()=>{const f=document.getElementById("education-page-form");if(!f)return;document.getElementById("add-education-semester").onclick=()=>document.getElementById("education-semesters-editor").appendChild(semesterEditor());f.onsubmit=async e=>{e.preventDefault();try{eduStatus("Saving…");const privacy={};document.querySelectorAll("[data-edu-privacy]").forEach(x=>privacy[x.dataset.eduPrivacy]=x.checked);const payload={schemaVersion:2,isPublic:document.getElementById("education-public").checked,publishStatus:eduVal("education-publish-status"),intro:eduVal("education-intro"),profile:{institution:eduVal("education-institution"),college:eduVal("education-college"),program:eduVal("education-program"),intendedProgram:eduVal("education-intended-program"),major:eduVal("education-major"),minor:eduVal("education-minor"),concentration:eduVal("education-concentration"),academicYear:eduVal("education-academic-year"),enrollmentStatus:eduVal("education-enrollment-status"),expectedGraduation:eduVal("education-expected-graduation")},degree:{name:eduVal("education-degree-name"),requiredCredits:eduNum("education-required-credits")||120},currentTerm:eduVal("education-current-term"),cumulativeGpa:eduNum("education-cumulative-gpa"),creditsCompleted:eduNum("education-credits-completed"),privacy,semesters:collectSemesters(),requirements:jsonRead("education-requirements-json"),timeline:jsonRead("education-timeline-json"),goals:jsonRead("education-goals-json"),plannedCourses:jsonRead("education-planned-json"),achievements:jsonRead("education-achievements-json"),certifications:jsonRead("education-certifications-json"),projects:jsonRead("education-projects-json"),skills:jsonRead("education-skills-json"),transferCredits:jsonRead("education-transfers-json"),updatedAt:serverTimestamp()};await setDoc(educationPageRef,payload,{merge:true});eduStatus("Advanced Education portfolio saved.")}catch(err){console.error(err);eduStatus(err.message||"Save failed.",true)}};document.getElementById("education-export-backup").onclick=()=>{const a=document.createElement("a");const data={profile:{institution:eduVal("education-institution"),program:eduVal("education-program")},semesters:collectSemesters()};a.href=URL.createObjectURL(new Blob([JSON.stringify(data,null,2)],{type:"application/json"}));a.download="education-backup.json";a.click()};document.getElementById("education-import-backup").onchange=async e=>{try{const d=JSON.parse(await e.target.files[0].text());renderAdvancedEducation(d);eduStatus("Backup loaded. Review and save to publish.")}catch(err){eduStatus("Invalid backup file.",true)}};loadAdvancedEducation()});
