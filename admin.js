...
// --- SMART CHECKBOX LOGIC FOR LEGISLATION PROGRESS ---
function setupLegislationCheckboxLogic() {
  ...
  checkboxes.forEach((checkbox, index) => {
    ...
    // Reset state and event listeners
...
async function loadLegislationAdmin() {
  ...
  try {
    ...
    // Reinitialize toggle logic after list loads
...
function populateLegislationForm(itemData) {
  ...
  // --- Handle both object or array formats for "status" ---
  let statusMap = {
    introduced: false,
    passedHouse: false,
    passedSenate: false,
  ...
  if (Array.isArray(itemData.status)) {
    itemData.status.forEach(key => { if (statusMap.hasOwnProperty(key)) statusMap[key] = true; });
  } else if (typeof itemData.status === 'object' && itemData.status !== null) {
    statusMap = { ...statusMap, ...itemData.status };
  }

  // Set checkbox states
  document.getElementById('status-introduced').checked = !!statusMap.introduced;
  document.getElementById('status-passed-house').checked = !!statusMap.passedHouse;
  document.getElementById('status-passed-senate').checked = !!statusMap.passedSenate;
  document.getElementById('status-to-president').checked = !!statusMap.toPresident;
  document.getElementById('status-became-law').checked = !!statusMap.becameLaw;

  // Scroll into view and reinit toggle logic
  setupLegislationCheckboxLogic();
  window.scrollTo({ top: addLegislationForm.offsetTop, behavior: 'smooth' });
}
