...
const button = document.getElementById("save-tiktok");
...
// Load current TikTok link when admin opens page
async function loadCurrent() {
  const snap = await getDoc(ref);
  if (snap.exists()) {
    const url = snap.data().latestTikTok || "";
    input.value = url;
    renderPreview(url);
  }
}
...
button.addEventListener("click", async () => {
  ...
  if (!isTikTokURL(url)) {
    statusEl.textContent = "Invalid TikTok link. Make sure it's a real video URL.";
    statusEl.style.color = "red";
    return;
  }

  await setDoc(ref, { latestTikTok: url }, { merge: true });
