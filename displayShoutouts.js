...
/* ========================================
   ...
   Full version with dynamic sub-status
   ======================================== */
...
/* ========================================
   displayShoutouts.js - Business Hours & Status
   FULL FILE — preserves overrides, temporary & holiday rules,
   visitor timezone conversion, and fixed "next open" logic
   ======================================== */
...
function formatDisplayTimeBI(timeString, visitorTimezone) {
  ...
  // If luxon not available, fall back to simple formatting + hint
  ...
  try {
    ...
    // create DateTime in business zone at that hour/minute on "today" in business timezone
...
function calculateAndDisplayStatusConvertedBI(businessData={}) {
  ...
  // ----------------------------
  // SUB-STATUS: Open till / Opens in / Opens tomorrow / Opens Monday...
  //  — Fixed logic: scans forward up to 7 days and respects closed days
  // ----------------------------
  function getMainStatusSubText(rule){
    const COUNTDOWN_WINDOW_MINUTES=30;
    let nowMinutes=currentMinutesInBizTZ;

    // build ranges to check for "Open till"
    const ranges = rule.ranges && rule.ranges.length ? rule.ranges : [{open:rule.open,close:rule.close}];
    if(!ranges || ranges.length===0) return '';

    // If currently open, show closing time for the active range
    for(let r of ranges){
    ...
    // Need to find the next actual opening time (could be same day later, or next open day)
    const orderedDays = ['sunday','monday','tuesday','wednesday','thursday','friday','saturday'];
    const todayIndex = orderedDays.indexOf(businessDayName);

    // Helper to get day object with ranges for a given day name
    function getDayObj(dayName){
      return regularHours[dayName] || { isClosed:true, ranges:[] };
    }

    // First, check later ranges today (if any) that start after now
    if(Array.isArray(todayRegular.ranges) && todayRegular.ranges.length){
      for(let r of todayRegular.ranges){
        const openM=timeStringToMinutes(r.open);
        if(openM===null) continue;
        if(openM > nowMinutes) {
          const minutesUntil = openM - nowMinutes;
          if(minutesUntil <= COUNTDOWN_WINDOW_MINUTES) return `Opens in ${minutesUntil} min`;
          return `Opens at ${formatDisplayTimeBI(r.open,visitorTimezone)}`;
        }
      }
    }

    // Otherwise scan next 6 days for the first actually open day with a valid range
    for(let offset=1; offset<8; offset++){
      const checkIndex = (todayIndex + offset) % 7;
      const checkDay = orderedDays[checkIndex];
      const dayObj = getDayObj(checkDay);

      if(!dayObj || dayObj.isClosed) continue; // skip closed days
      const dayRanges = Array.isArray(dayObj.ranges) ? dayObj.ranges : (dayObj.open && dayObj.close ? [{open: dayObj.open, close: dayObj.close}] : []);
      if(!dayRanges.length) continue;

      // Use the first range's start as canonical "opens" for that day
      const firstRange = dayRanges[0];
      const openM = timeStringToMinutes(firstRange.open);
      if(openM===null) continue;

      // minutes until that next opening (account for day offset)
      const minutesUntil = (openM + offset*1440) - nowMinutes;

      if(minutesUntil <= COUNTDOWN_WINDOW_MINUTES) return `Opens in ${minutesUntil} min`;
      if(offset === 1) return `Opens tomorrow at ${formatDisplayTimeBI(firstRange.open,visitorTimezone)}`;
      return `Opens ${capitalizeFirstLetter(checkDay)} at ${formatDisplayTimeBI(firstRange.open,visitorTimezone)}`;
    }

    // No future opening found
    return '';
  }
  ...
  // ----------------------------
  // RENDER REGULAR HOURS (visitor-local times)
  // ----------------------------
