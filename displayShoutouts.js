...
function checkDeviceSupport(item) {
    ...
    if (osStatus?.status === "Very Outdated") {
        ...
        supportLevel = "Unsupported";
        supportColor = "red";
    } else if (osStatus?.isBehindPublic && supported) {
...
// ======================
// CURRENT AI FEATURE SUPPORT.// CURRENT AI FEATURE SUPPORT
// ======================
function calculateAIFeatureSupport(item) {
    ...
    if (deviceType === "phone") {
        ...
        if (isIPhone15Pro || isIPhone16OrNewer || chip.isA17ProOrNewer) {
            ...
            score = 70;
            color = "yellow";
            reasons.push("Meets the current Apple Intelligence hardware class.");
        }
        ...
        if (level === "Standard" && ramGB > 0 && ramGB < 12) {
            weaknesses.push("8GB memory supports current Apple Intelligence but may not qualify for future advanced on-device AI tiers.");
        }
    }
...
// ======================
// FUTURE AI TARGET
// This judges what the future AI Target: Maximum// This judges what the future upgrade should aim for.
// ======================
...
function calculateUpgradeScore(item) {
    ...
    if (support.supportLevel === "Older Device") {
        score -= 14;
    }

    if (support.supportLevel === "Support Ending Soon") {
        score -= 12;
    }

    if (support.supportLevel === "Support Ending Next Year") {
        score -= 8;
    }
    ...
    if (deviceType === "accessory") {
        if (condition === "needs repair") {
            return {
                year: currentYear,
                window: `${currentYear}`,
                timing: "Replace now if this accessory is still needed."
            };
        }
...
function generateUpgradeExplanation(item, priority, recommendedUpgrade, upgrade, support, osStatus, upgradeScore) {
    ...
    if (condition === "Excellent" || condition === "Good") {
        reasons.push(`Condition is ${condition.toLowerCase()}`);
    }

    if (support.supportLevel) {
        reasons.push(`Support status is ${support.supportLevel.toLowerCase()}`);
    }
    ...
    if (batteryHealth >= techThresholds.batteryBad) {
        reasons.push("Battery health is still acceptable");
    }

    if (cycles < techThresholds.cycleOld) {
        reasons.push("Charge cycles are below your upgrade threshold");
    }
...
function getRecommendedAction(item, upgrade, support, osStatus) {
    ...
    if (condition === "retired" || support.supportLevel === "Retired") {
        return "No active upgrade needed";
    }

    if (condition === "needs repair") {
        return "Repair or replace depending on cost";
    }
...
function generateDeviceSummary(item, upgrade, support, osStatus) {
    ...
    } else if (upgrade.status === "Retired") {
        parts.push("no active upgrade is needed");
    } else {
...
function renderTechItemHomepage(itemData) {
    ...
    const modelYear = itemData.modelYear || "";
    const supportEndYear = itemData.supportEndYear || "";
    ...
    const rawBatteryCycles = itemData.batteryCycles ?? itemData.batteryChargeCycles;
    ...
    const recommendedAction = getRecommendedAction(itemData, upgrade, support, osStatus);
    ...
    const deviceSummary = generateDeviceSummary(itemData, upgrade, support, osStatus);
...
async function loadRegionalLeader() {
  ...
  const setLoading = () => {
    ...
    if (govLabelEl) {
      govLabelEl.textContent = "Governor";
    }
  ...
  const commonsFileToUrl = (value, width = 256) => {
    ...
    return `https://commons.wikimedia.org/wiki/Special:FilePath/${encodeURIComponent(stringValue)}?width=${width}`;
  };
  ...
  async function resolveWikiLabel(value) {
    ...
    if (wikidataLabelCache[qid]) {
      return wikidataLabelCache[qid];
    }
  ...
  async function getLeaderDisplayName(leader) {
    if (!leader) return "Not listed";

    if (typeof leader === "string") {
      return await resolveWikiLabel(leader);
    }
    ...
    const rawName =
      ...
      leader.itemLabel ||
      leader.personLabel ||
      leader.officeholderLabel ||
      leader.presidentLabel ||
      leader.governorLabel ||
      leader.leaderLabel ||
      leader.id ||
      leader.item ||
      leader.person ||
    ...
    return await resolveWikiLabel(rawName);
  }
  ...
  async function detectGeo() {
    ...
    for (const provider of providers) {
      try {
        const response = await safeFetch(provider.url, {}, 9000);
  ...
  async function fetchCurrentNationalLeader(countryQid) {
    const query = `
      SELECT ?leader ?leaderLabel ?image ?start ?end WHERE {
        wd:${countryQid} p:P35 ?statement.
  ...
  async function fetchCurrentRegionalLeader(regionQid) {
    const query = `
      SELECT ?leader ?leaderLabel ?image ?start ?end WHERE {
        wd:${regionQid} p:P6 ?statement.
  ...
  function setLeaderImage(imgEl, imageValue, altText) {
    if (!imgEl) return;

    const imgUrl = commonsFileToUrl(imageValue, 256);
    ...
    imgEl.src = imgUrl;
    imgEl.alt = altText || "Leader image";
    imgEl.style.display = "";
  }
  ...
  async function renderLeaders() {
    ...
    try {
      ...
      subtitleEl.textContent = geo.stateCode
        ? `Detected region: ${geo.stateCode}, ${geo.countryName}`
        : `Detected region: ${geo.countryName}`;
      ...
      if (president) {
        const presidentName = await getLeaderDisplayName(president);
        presNameEl.textContent = presidentName || "Not listed";
        presTermEl.textContent = formatTerm(president.start, president.end);
        setLeaderImage(presImgEl, president.image, presidentName);
      } else {
        presNameEl.textContent = "Not listed";
        presTermEl.textContent = "Term: Not listed";
      ...
      if (geo.countryCode === "US" && geo.stateCode && US_STATE_CODE_TO_QID[geo.stateCode]) {
        ...
        if (govLabelEl) {
          govLabelEl.textContent = `${geo.stateCode} Governor`;
        }

        if (governor) {
          const governorName = await getLeaderDisplayName(governor);
          govNameEl.textContent = governorName || "Not listed";
          govTermEl.textContent = formatTerm(governor.start, governor.end);
          setLeaderImage(govImgEl, governor.image, governorName);
        } else {
          govNameEl.textContent = "Not listed";
          govTermEl.textContent = "Term: Not listed";
          setLeaderImage(govImgEl, null, "");
        }
      } else {
        if (govLabelEl) {
          govLabelEl.textContent = "Regional Leader";
        }
      ...
      console.error("Regional leader load failed:", error);

      subtitleEl.textContent = "Unable to load regional leaders.";
      footnoteEl.textContent = "Source unavailable.";
