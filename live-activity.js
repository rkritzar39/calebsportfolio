...
/**
 * Live Activity – FINAL STABLE BUILD
 * - One main persistent icon (Spotify → Discord → Manual)
 * - Temp icons (TikTok, Reddit, GitHub, Twitch) fade out
 * - No duplicate Spotify icons
 * - Shows “Offline” and clears icons when idle
 */
...
const ICONS = {
  ...
  tiktok: `<svg viewBox="0 0 24 24"><path fill="#EE1D52" d="M12.5 0c.3 2.3 1.8 4.3 3.9 5.2V0h2.4v14.6c0 4.4-3.6 8-8 8-4.4 0-8-3.6-8-8 0-3.8 2.7-7 6.3-7.8v2.5c-2.2.7-3.8 2.7-3.8 5.2 0 3 2.5 5.5 5.5 5.5s5.5-2.5 5.5-5.5V7.8c-.9-.3-1.8-.8-2.6-1.4V0h-1.2z"/></svg>`,
...
function setStatusLine(text, src = "manual") {
  ...
  ico.innerHTML = src !== "manual" ? renderIcon(src) : "";
...
function updateIconCluster(results) {
  ...
  temps.forEach((r) => {
    const el = document.createElement("span");
    el.className = `cluster-icon-svg temp-icon ${r.source} active`;
  ...
  // hide all icons if nothing active
  if (!main && temps.length === 0) {
    cluster.innerHTML = "";
  }
}
...
async function getDiscord() {
  try {
    ...
    setStatusLine(status, data.discord_status === "offline" ? "manual" : "discord");
...
async function getReddit() {
  try {
    ...
    if (json?.data?.children?.length)
      return { text: "Shared on Reddit", source: "reddit" };
  } catch {}
  return null;
}
...
async function getGitHub() {
  try {
    ...
    if (evt) return { text: "Committed on GitHub", source: "github" };
  } catch {}
  return null;
}
...
async function updateLiveStatus() {
  ...
  const main =
    results.find((r) => ["spotify", "discord"].includes(r.source)) || results[0];

  if (main) setStatusLine(main.text, main.source);
  else setStatusLine("Offline", "manual");
